...
use livenet_common::{
    gas_limit, load_settlement, optional_u64, parse_address, parse_public_key, required_value,
    DEFAULT_CURRENCY, DEFAULT_INSURED_VALUE_MINOR, DEFAULT_POLICY_ID,
};
...
fn main() {
    ...
    let claimant_value = env::var("UNDERWRITE_SELF_CLAIMANT_ACCOUNT")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .unwrap_or_else(|| required_value("DEMO_CLAIMANT_ACCOUNT", "claimantAccount"));
    let claimant = parse_address("UNDERWRITE_SELF_CLAIMANT_ACCOUNT", &claimant_value);
    let insured_value_minor = optional_u64(
    ...
    settlement.register_policy_self(
        policy_id.clone(),
        claimant,
        insured_value_minor,
        currency.clone(),
    ...
    println!("PolicySelfRegistrationStatus: submitted");
